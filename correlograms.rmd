...
### Generate and save a specific missing correlogram
--- Generate and Save Q|SWIT for chosen period
```{r}

num <-  4 #select site by number
wateryear <- 2016
Cor.code <- '_F_Q_C'
...
#msd <- which(wydf$wyday==comp.df[site,paste0(year.char,'_MSD')]-30)
#rsd <- comp.df[site,paste0(year.char,'_RSD')]

which(wydf$day=='6-Sep') #use ctrl+Enter to look up index for a particular day
a <- 142 #start row for correlation
b <- 331 #end row for correlation 341
acf.code <- paste0(siteabv,'_',year.char,Cor.code)


plot(wydf$qpatch, type = 'l', col = linecols[num], main = acf.code)
rect(a,0.05*max(wydf$qpatch, na.rm = T), b,max(wydf$qpatch, na.rm = T))
```
...
ccf1 <- ccf(flow.per$qpatch, flow.per$SWIT, lag.max = lagmax, 
             plot = F, na.action = na.pass)

lagmax <- max(ccf1$lag)

acf.df <- data.frame(lag=ccf1$lag)
acf.df[acf.code] <- ccf1$acf
acf.df <- acf.df[which(acf.df$lag>=0),]

plot(acf.df$lag, acf.df[,acf.code], xlim = c(0,50), type = 'l', col = linecols[num], main = paste(acf.code, acf.code %in% colnames(corr.df)))
abline(h = 0.2, lty = 3)
```
"False" in title indicates the given correlogram is NOT in corr.df.  Now add it here:
...
paste(acf.code,'added to corr.df at',date())
```


---Generate and Save Q ACF for chosen period
```{r}
...
which(wydf$day=='02-02') #use ctrl+Enter to look up index for a particular day
which(wydf$day=='09-25') #use ctrl+Enter to look up index for a particular day

a <- 120 #start row for correlation
b <- 360 #end row for correlation 341
acf.code <- paste0(siteabv,'_',year.char,Cor.code)
...
acf1 <- acf(flow.per$qpatch, lag.max = lagmax, 
             plot = F, na.action = na.pass)

lagmax <- max(acf1$lag)

acf.df <- data.frame(lag=acf1$lag)
acf.df[acf.code] <- acf1$acf
acf.df <- acf.df[which(acf.df$lag>=0),]
...
---
### Use the following section to evaluate correlation to snotel data (can't be added to corr.df yet) and plot C|Q ccf
...
--- add water year columns if necessary
```{r}
...
--- Evaluate CCF functions of C|Q over pre-determined intervals (from sensitivity.rmd)
```{r}
crossdf <- qtc
...
```
*optional - check that a given index correctly corresponds to the desired date.  This is relevant when using pre-established indices to determine windows for analysis
```{r}
i.check <- 1369
...
#plot conductance over the window.  The line gets plotted later over the highlight rectangles
...
--- Generate and Save Q|SWIT for melt period
```{r}
...
acf.code <- paste0(siteabv,'_',year.char,'_M_Q_C')
acf.df <- data.frame(lag=mccf$lag)
acf.df[acf.code] <- mccf$acf
acf.df <- acf.df[which(acf.df$lag>=0),]
corr.df <- merge(corr.df, acf.df, by = 'lag', all = T)
}
