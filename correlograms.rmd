...
The next section generates ACF functions
---
...
This section generates and plots CCF functions from SNOTEL data
---
...
This section generates and plots CCF functions from UEB model results
---
...
```{r}
lagmax <- 360
c_by_swi <- ccf(crossdf$cpatch,crossdf$SWIT, lag.max = lagmax, plot = F, na.action = na.pass)
q_by_c <- ccf(crossdf$qpatch,crossdf$cpatch, lag.max = lagmax, plot = F, na.action = na.pass)
q_by_swi <- ccf(crossdf$qpatch,crossdf$SWIT, lag.max = lagmax, plot = F, na.action = na.pass)
```

Create on plot of all calculated cross-correlations
```{r}
par( mar = c(5.1,4.1,4.1,8.1))
plot(c_by_swi$acf~c_by_swi$lag, xlab='lag (days)', ylab='correlation',
     col=linecols[1], type = 'l', ylim=c(-0.6,0.6), xlim = c(0,lagmax))
lines(q_by_c$acf, col = linecols[2])
lines(q_by_swi$acf, col = linecols[3])

par(xpd=T)
title(main = c("Cross-correlation function for",sitename))
legend('topright', col = linecols, lty = 1, inset = c(-.3,0),
       legend = c('SC | SWIT', 'Q | SC', 'Q | SWIT'))
```
```{r}
corrhold <- 0.20
paste('Lag times for end of correlation at',sitename)
