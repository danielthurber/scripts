...
```{r}
study.int <- c(1400,1600) #select the study interval here
date.mark <- '02-15' #vertical yellow lines will be drawn at this date for reference
...
abline(v = which(format(qtc$date,'%m-%d')==date.mark), col='goldenrod')
```
Select a specific study interval targeting one runoff season from plot above.  This will isolate a water year and plot discharge. 
```{r}
study.int <- c(1650,1855) #redefine the study interval, or do so by re-running the chunk above
...
plot(qtc$qpatch, xlim = study.int, ylim = yrange, type = 'l', ylab = 'discharge', 
     main = paste('discharge at', sitename, qtc$WY[a], '-', qtc$WY[b-60]), xaxt = 'n',
     xlab = 'Date')
...
plot(acf1$lag, acf1$acf, type = 'l', col=linecols[1],
     ...
     main = paste('Q|SWIT Crosscorrelation window sensitivity:', sitename, qtc$WY[a]))
