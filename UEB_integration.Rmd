...
```{r}
mod.df$WY = ifelse(as.numeric(format(mod.df$date,'%m'))>=10, 
                     as.numeric(format(mod.df$date,'%Y'))+1, 
                     as.numeric(format(mod.df$date,'%Y'))) #determine water year
```

```{r}
plot(mod.df$cpatch, type = 'l', col = 'red', xlim = c(0,))
lines(qtc$cpatch)
