...
```{r}
plot(crossdf$qpatch, type = 'l')
```


---Trim end of dataframe

select an index range (based on plot above) and see if the interval looks good
```{r}
keep.rows <- 1:nrow(crossdf)
#keep.rows <- 1:2380 #uncomment to keep all
plot(crossdf$cpatch[keep.rows], type = 'l')
plot(crossdf$qpatch[keep.rows], type = 'l', xlim = range(keep.rows))
abline(v = which(is.na(crossdf$qpatch[keep.rows])), col = 'red')

```

Trim the dataframe and plot result.  Unpatched values are indicated.
```{r}
new.cross <- crossdf[keep.rows,]
plot(new.cross$qpatch, type = 'l')
...
```{r}
plot(new.data$discharge, type = 'l')
...
check for present/missing columns.  all columns from append should be in new.cross
```{r}
